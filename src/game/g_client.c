  ...
  *p = 0;
  ...
  while( 1 )
  {
    ...
    if( ch == Q_COLOR_ESCAPE )
    {
      ...
      *out++ = ch;
      // don't allow black in a name, use white, Devils
      if( !ColorIndex( *in ) && !g_allowClientBlackNames.integer )
        *out++ = '7';
      else
        *out++ = *in;
      in++;
...
void ClientBegin( int clientNum )
{
  ...
  G_DemoCommand( DC_CLIENT_SET, va( "%d %s", clientNum, userinfo ) );
    ...
    if( g_clientUpgradeNotice.integer )
    {
       ...
       {
           trap_SendServerCommand( client->ps.clientNum, va( "print \"^7English: ^1Your client is out of date. Updating your client will allow you to "
                                                             "become an admin on servers and download maps much more quickly. Please replace your client executable with the one "
                                                             "at ^5http://www.tremulous.net.ar/descargas^1 and reconnect.\n \"" ) );

           trap_SendServerCommand( client->ps.clientNum, va( "print \"^7Portugues: ^6O seu cliente esta desatualizado. Atualizando o seu cliente voce podera se tornar um administrador "
                                                           "no servidor e fazer download de mapas muito mais rapido. Por favor, substitua o seu cliente por esse "
                                                           "^5http://www.tremulous.net.ar/descargas^6 e se reconecte.\n \"" ));

           trap_SendServerCommand( client->ps.clientNum, va( "print \"^7Espanol: ^2Tu cliente esta desactualizado. Actualizar tu cliente te permitira "
                                                           "ser administrador en los servidores y descargar mapas mucho mas rapido. "
                                                           "Por favor, descarga e instala el que se encuentra en ^5http://www.tremulous.net.ar/descargas^2 y vuelve a conectarte.\n \"" ));
