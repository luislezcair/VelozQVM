...
static int rotationVoteCount[ MAX_MAP_ROTATION_CONDS ];
...
qboolean G_IntermissionMapVoteWinner( void )
{
  ...
  G_GetMapVoteWinner( &winvotes, &totalvotes );
...
static void G_IntermissionMapVoteMessageReal( gentity_t *ent, int winner, int winvotes, int totalvotes )
{
  ...
  for( i = 0; i < rotationVoteLen; i++ )
  {
    ...
    if( rotationVoteCount[ i ] != 0 )
        Com_sprintf( voteCount, sizeof( voteCount ), "%d", rotationVoteCount[ i ] );
    else
        Com_sprintf( voteCount, sizeof( voteCount ), "%s", "" );

    Com_sprintf( entry, sizeof( entry ), "^7%s%s%s %s %s^7%s   %s\n",
     ( i == index ) ? "^1>>>" : "",
     ( i == selection ) ? "^7(" : " ",
     color,
     rotationVoteList[ i ],
  ...
  Com_sprintf( entry, sizeof( entry ),
    "^7+----------------+\nLeader: ^3%s^7 with %d vote%s of %d voter%s\nTime left: %d",
...
void G_IntermissionMapVoteCalculate( void )
{
    int n;
    int i;
    ...
    for( i = 0; i < MAX_MAP_ROTATION_CONDS; i++ )
    {
        rotationVoteCount[ i ] = 0;
    }

    for( i = 0; i < MAX_CLIENTS; i++ )
    {
        n = rotationVoteClientSelection [ i ];
        rotationVoteCount[ n ] += 1;
    }
}
...
void G_IntermissionMapVoteCommand( gentity_t *ent, qboolean next, qboolean choose )
{
  ...
  if( choose )
  {
    rotationVoteClientSelection[ clientNum ] = n;
    G_IntermissionMapVoteCalculate( );
  }
  else
  {
    if( next )
