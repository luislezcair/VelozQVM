...
void ClientTimerActions( gentity_t *ent, int msec )
{
  ...
  while( client->time1000 >= 1000 )
  {
    ...
    //radioactive turrets (update timer)
    if ( client->ps.stats[ STAT_PTEAM ] == PTE_HUMANS && g_radiationDamage.integer && !( ent->flags & FL_NOTARGET ) )
    {
      int i;
      int credits = 0;

      //check if client has enough equipment/credits to be penalized for camping
      if ( g_radiationCreditsCount.integer == 0 || g_radiationCreditsCount.integer == 2 )
        credits += BG_GetValueOfEquipment( &client->ps );
      if ( g_radiationCreditsCount.integer == 1 || g_radiationCreditsCount.integer == 2 )
        credits += client->ps.persistant[ PERS_CREDIT ];

      //check for any turrets/teslas in range
      if ( credits >= g_radiationCredits.integer && client->ps.weapon != WP_HBUILD && client->ps.weapon != WP_HBUILD2 )
      {
        ...
        for( i = 0; i < num; i++ )
        {
          turret = &g_entities[ entityList[ i ] ];
          if ( turret->s.modelindex == BA_H_MGTURRET && turret->powered && turret->spawned && turret->health && !( turret->s.eFlags & EF_FIRING ) && Distance( turret->s.pos.trBase, ent->s.pos.trBase ) <= MGTURRET_RANGE )
            foundTurrets++;
        }
        ...
        {
          turret = &g_entities[ entityList[ i ] ];
          if ( turret->s.modelindex == BA_H_TESLAGEN && turret->powered && turret->spawned && turret->health && turret->dcced && !( turret->s.eFlags & EF_FIRING ) && Distance( turret->s.pos.trBase, ent->s.pos.trBase ) <= TESLAGEN_RANGE )
            foundTurrets++;
        }

        if ( foundTurrets )
        {
          if ( client->radiationTimer < g_radiationTime.integer )
            client->radiationTimer++;
        }
        else if ( client->radiationTimer > 0 )
          client->radiationTimer--;
      }
      else if ( client->radiationTimer > 0 )
  ...
  while( client->time10000 >= 10000 )
  {
    ...
    //radioactive turrets (damage)
    if ( g_radiationDamage.integer && foundTurrets && client->radiationTimer == g_radiationTime.integer )
    {
      trap_SendServerCommand( ent-g_entities, "cp \"^1You have been irradiated by your base^7\"" );
      G_Damage( ent, NULL, NULL, NULL, NULL, g_radiationDamage.integer * foundTurrets, DAMAGE_NO_ARMOR|DAMAGE_NO_LOCDAMAGE, MOD_RADIATION );
    }
  }
}
